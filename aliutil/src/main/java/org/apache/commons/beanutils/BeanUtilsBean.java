...
import com.shenma.aliutil.util.ListTypeConverter;
...
public class BeanUtilsBean {
    ...
    public void setProperty(Object bean, String name, Object value)
        ...
        if (type.isArray() && (index < 0)) { // Scalar value into array
            ...
            if (value instanceof String) {
	            ...
	            if(name!=null){
	            	try {
	            		Type t = bean.getClass().getDeclaredField(name).getGenericType();
	            		 /*for (Type t1:((ParameterizedType)t).getActualTypeArguments()) {          
	                         t=t1;          
	                     } */  
	            		 Type[] tArgs=((ParameterizedType)t).getActualTypeArguments();
						 newValue = convert(value, type,tArgs);
    ...
    protected Object convert(Object value, Class<?> type,Type[] paramType) {
        Converter converter = getConvertUtils().lookup(type);
        if (converter != null) {
            log.trace("        USING CONVERTER " + converter);
            ListTypeConverter lconverter=(ListTypeConverter) converter;

            return lconverter.convert(type, value,paramType);
        } else {
            return value;
        }
    }
